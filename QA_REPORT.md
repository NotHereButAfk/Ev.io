...
**Verification:** PASS in production. The real `soldier.glb` was rendered from front, three-quarter, and side views at the production 18% idle blend. All 17 firearms pass 816 loaded-Soldier pose/armor/action combinations with 0.00 cm wrist error, 0.0 cm torso/shoulder penetration, at least 21.7 cm lateral receiver clearance, and at least 31.1 cm forward clearance. The fallback reference measures 19.4 cm firing-elbow drop and 12.2 cm support-elbow drop. Human locomotion, firearm clearance, action continuity, production build, and all 28/28 automated release gates pass. Loaded-Soldier commit `9e34159` deployed in run `31922690600`; fallback elbow follow-up `98d434c` deployed on the successful retry of run `31922876801`. A cache-busted `kryx.live` session entered the live 8/8-player match in third person with zero page errors.
...
**Verification:** PASS in production. All three connected bodies retain the 1.822 m human figure, animation bones, full skin weighting, outward-facing surfaces, planted soles, and valid headshot height. All 17 firearms still pass 272 production carry/action poses with 0.00 cm hand error, 0.0 cm torso penetration, and no more than 0.6 cm shoulder contact. M4 front, three-quarter, side, and rear renders show the slim shoulders, continuous mask, eye slit, and visible scarf tails with zero browser errors. Commit `3b66788` deployed successfully in run `31896838927`; a cache-busted `kryx.live` replay entered the 8/8 match, switched to third person, rendered the dark hood and twin scarf tails with the M4 outside the torso, and recorded zero page errors.
...
**Verification:** PASS in production. The updated gate rejects any firearm without an explicit profile. All 17 firearms pass 272 production carry/action poses with 0.00 cm trigger/support wrist error, 0.0 cm torso penetration, and at most 0.6 cm armor-shoulder contact. Rifle reference, first-person viewmodel, and complete action-continuity suites pass. Front/three-quarter/side renders were inspected for pistols, SMG, shotgun, M4, LMG, sniper, RPG, fuel-rod launcher, and needler; the category-specific soldier silhouettes and physical palm contacts are visible. Real-browser gameplay smoke reports zero console/request failures, and all 28/28 automated release gates pass. Commit `4cb7b7a` deployed successfully in run `31895617661`; a cache-busted `kryx.live` replay entered the 8/8 match, accepted movement, jump, and reload input, rendered the corrected M4 in first person, switched to the connected third-person soldier with the receiver outside the torso and stock seated at the shoulder, and recorded zero page errors.
