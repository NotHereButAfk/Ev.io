...
requireMatch(game, /async _runConnectSequence\(\)[\s\S]*?findAvailableMatch[\s\S]*?_showMapLoading[\s\S]*?startInitialLoad\([\s\S]*?_finishMapLoading[\s\S]*?_setStartupProgress\(['"]READY['"],\s*100[\s\S]*?_initAuth\(\)[\s\S]*?_schedulePresentationPreloads/,
  'startup exposes READY and menu only after real arena loading, before optional presentation downloads');
